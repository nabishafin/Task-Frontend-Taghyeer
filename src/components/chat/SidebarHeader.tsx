    ...
    <div className="p-3.5 px-4 border-b border-slate-200 bg-white flex items-center justify-between gap-3 shrink-0">
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0 flex-1">
          <h2 className="text-sm font-bold text-slate-900 truncate">{user?.name || 'User'}</h2>
          <p className="text-xs text-slate-500 truncate flex items-center gap-1">
            <span className="truncate">{user?.phone}</span>
            {socketConnected ? (
              <span className="inline-flex items-center gap-1 text-[10px] font-bold text-[#2d8a2d] bg-[#88E788]/25 px-1.5 py-0.2 rounded">
                <Wifi className="w-2.5 h-2.5" /> Live
              </span>
            ) : (
              <span className="inline-flex items-center gap-1 text-[10px] font-medium text-amber-700 bg-amber-50 px-1.5 py-0.2 rounded border border-amber-200">
