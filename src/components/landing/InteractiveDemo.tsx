    ...
    <section id="interactive" className="py-12 md:py-16 bg-slate-50/70 text-slate-900 border-b border-slate-200/80 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center space-y-3 max-w-3xl mx-auto mb-8">
          <div className="inline-flex items-center gap-1.5 px-3.5 py-1 rounded-full bg-[#88E788]/20 border border-[#88E788]/50 text-slate-900 text-xs font-bold uppercase tracking-wide shadow-2xs">
        ...
        {/* Integrated Sandbox Display - Uniform Subtle Edges */}
        <div className="max-w-6xl mx-auto bg-white border border-slate-200 rounded-xl p-4 sm:p-5 shadow-xs space-y-4">
          <div className="flex flex-wrap gap-2 pb-3.5 border-b border-slate-100">
            {PRESET_QUESTIONS.map((q, idx) => (
              <button
                key={idx}
                onClick={() => handleSend(q)}
                className="text-xs font-semibold text-slate-800 bg-slate-100 hover:bg-[#88E788] hover:text-slate-950 px-3 py-1.5 rounded transition-all border border-slate-200"
              >
                {q}
              </button>
            ))}
